...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const dup = await checkIdempotency(supabase, idempotency_key, employee_id, "project-end-break");
    if (dup) return dup;
    ...
    const { error } = await supabase
      ...
      .update({
        break_end_time: now,
        break_minutes: Math.min(MAX_BREAK_MINUTES, (session.break_minutes ?? 0) + addBreak),
      })
      .eq("id", session_id);

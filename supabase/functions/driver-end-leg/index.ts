...
Deno.serve(async (req) => {
  ...
  try {
    const { employee_id, client_timestamp, idempotency_key, leg_id, lat, lng } = await req.json();
    ...
    const dup = await checkIdempotency(supabase, idempotency_key, employee_id, "driver-end-leg");
    if (dup) return dup;

...
import { Loader2, MapPin, Clock, Wifi, WifiOff, CheckCircle2, AlertTriangle } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { useState, useEffect } from "react";

export default function MobileHome() {
  const { employee } = useMobileAuth();
  const { step, assignment, availableActions, loading, actionLoading, executeAction } = useMobileWorkflow();
  ...
  const handleAction = async (action: WorkflowAction) => {
    // For GPS actions, get current position
    let payload: Record<string, unknown> = {};

    const gpsActions: WorkflowAction[] = ["punch_in", "start_travel", "arrive_site"];
    if (gpsActions.includes(action)) {
      try {
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              {assignment.siteAddress && (
                <p className="text-sm text-muted-foreground truncate">{assignment.siteAddress}</p>
              )}
              {assignment.shiftStart && assignment.shiftEnd && (
                <div className="flex items-center gap-1.5 mt-1.5">
                  <Clock className="h-3.5 w-3.5 text-muted-foreground" />
                  <span className="text-xs text-muted-foreground">
                    {assignment.shiftStart?.slice(0, 5)} – {assignment.shiftEnd?.slice(0, 5)}
                  </span>
                </div>
              )}
            </div>
          ...
          {(["idle", "punched_in", "traveling", "at_site", "working", "work_done", "punched_out"] as const).map((s, i) => {
            ...
            return (
              <div key={s} className="flex-1 flex flex-col items-center gap-1">
                <div
                  className={`h-2 w-full rounded-full ${isComplete ? "bg-brand" : "bg-muted"} ${isCurrent ? "ring-2 ring-brand/30" : ""}`}
                />
              </div>
            );
          })}

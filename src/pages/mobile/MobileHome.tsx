...
import { useTodayProjects, projectWorkedMinutes } from "@/hooks/useTodayProjects";
...
export default function MobileHome() {
  ...
  return (
    <div className="flex flex-col gap-4 px-4 pb-24 safe-area-inset" style={{ paddingTop: 'calc(env(safe-area-inset-top, 0px) + 3rem)' }}>
      ...
      {/* Refresh row — full width so users see it clearly */}
      <button
        type="button"
        onClick={handleRefreshToday}
        disabled={projectsFetching}
        className="w-full inline-flex items-center justify-center gap-2 h-11 rounded-lg border border-brand/40 bg-brand/10 px-3 text-sm font-semibold text-brand transition-colors hover:bg-brand/20 disabled:opacity-60"
        aria-label="Refresh today's projects"
      >
        <RotateCcw className={`h-4 w-4 ${projectsFetching ? "animate-spin" : ""}`} />
        {projectsFetching ? "Refreshing…" : "Refresh today's data"}
      </button>
      ...
      {step !== "idle" && step !== "punched_out" && !isDriverDay && !!todayProjects?.length && (
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            ...
            <p className={`text-xs ${allProjectsDone && !allInHouseDay ? "text-amber-400" : "text-muted-foreground"}`}>
              {allProjectsDone
                ? (allInHouseDay ? "Work ended · ready to punch out" : "Work ended · office return pending")

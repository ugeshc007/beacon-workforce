...
import { useAssignToMaintenance } from "@/hooks/useMaintenance";
...
export function MaintenanceAssignDialog({ open, onOpenChange, maintenanceCallId, existingEmployeeIds }: Props) {
  ...
  const assignMutation = useAssignToMaintenance();
  ...
  const handleAssign = async (employeeId: string) => {
    try {
      await assignMutation.mutateAsync({
        maintenance_call_id: maintenanceCallId,
        employee_id: employeeId,
        date,
        shift_start: shiftStart,
        shift_end: shiftEnd,
      });
      toast({ title: "Staff assigned", description: "Successfully added to maintenance call." });
    } catch (err: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Assign Staff to Maintenance</DialogTitle>
        </DialogHeader>
        ...
        <ScrollArea className="h-[300px] -mx-1 px-1">
          {isLoading ? (
            ...
            <div className="space-y-1">
              {available.map((emp) => (
                <div key={emp.id} className="flex items-center justify-between p-2.5 rounded-lg hover:bg-muted/50 transition-colors">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2">
                        <Badge variant="outline" className="text-[10px]">{emp.skill_type === "team_member" ? "Member" : "TL"}</Badge>
                        <span className="text-[10px] text-muted-foreground font-mono">{emp.employee_code}</span>
                      </div>
                    </div>
                  </div>
                  <Button size="sm" variant="outline" className="shrink-0 h-7 text-xs gap-1" disabled={assignMutation.isPending} onClick={() => handleAssign(emp.id)}>
